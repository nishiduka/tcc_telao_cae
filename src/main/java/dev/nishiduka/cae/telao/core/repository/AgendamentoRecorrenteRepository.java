...
public interface AgendamentoRecorrenteRepository extends JpaRepository<AgendamentoRecorrenteEntity, Long> {
    ...
    @Query("SELECT CASE WHEN COUNT(a) > 0 THEN true ELSE false END FROM AgendamentoRecorrenteEntity a WHERE a.id != :#{#a.id} AND a.diaSemana = :#{#agendamento.diaSemana} AND a.sala.id = :#{#agendamento.sala.id} AND ((a.horaInicio < :#{#agendamento.horaFim} AND a.horaFim > :#{#agendamento.horaInicio}))")
